...
public class DeckManager {
	...
	public static void main(String args[]) {
		...
		UnitCard grizz = new UnitCard("Grizzly", 3, 4, 6,
			...
			try {
				...
				case 8:
					...
					if (decks.get(deckRem).existsInDeck(cards.get(cardRem))) {
						...
						System.out.println("Removed " + quantRem + " " + plural2 + " of " + cards.get(cardRem)
								+ " from " + decks.get(deckRem).getDeckName() + ".");
						System.out.println(
								cards.get(cardRem) + " is no longer in " + decks.get(deckRem).getDeckName() + ".");
					} else {
